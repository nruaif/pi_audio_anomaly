        ...
        if is_warming_up:
            ...
            if not tui_q.full():
                tui_q.put_nowait({"anomaly_score": 0.0, "warmup": True,
                                  "status": f"Building Coreset ({len(history_embeddings)}/{WARMUP_CHUNKS})"})

            if len(history_embeddings) >= WARMUP_CHUNKS:
                features_np = np.array(history_embeddings, dtype=np.float32)
                memory_bank = greedy_coreset_subsampling(features_np, fraction=0.2)
                ...
                del history_embeddings, features_np
                gc.collect()
